...
Name: Edward Wang, Yangzi He
Login: cs61a-ak, cs61a-am
TA: Keegan Mann
Section: 26
"""
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        """Takes an other ant as an argument and returns True if and only if:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        curr_loc = self.place
        count_range = 0
        while count_range < self.min_range:
            curr_loc = curr_loc.entrance
            count_range += 1
        while curr_loc.name != 'Hive':
            if curr_loc.bees != []:
                return random_or_none(curr_loc.bees)
            if count_range > self.max_range:
                return None
            curr_loc = curr_loc.entrance
            count_range += 1
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        list_of_bees = self.place.bees[:]
        for each_bee in list_of_bees:
            each_bee.reduce_armor(NinjaAnt.damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.queen_number = QueenAnt.number_of_queens
...
class QueenPlace(object):
    ...
    @property
    def bees(self):
        return self.bees1.bees + self.bees2.bees
...
def make_slow(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    def time_helper(time_left):
        def col_helper(colony):
            nonlocal time_left
            if time_left <= 0 or colony.time % 2 == 0:
                time_left -= 1
